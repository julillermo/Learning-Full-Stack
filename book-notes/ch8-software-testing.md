...
- Types of software tests mentioned:
  ...
  - **End-to-End Tests**
  - **Snapshot Tests**
- This chapter attempts simulate the process of **test-driven development** (TDD) for the reader
...
- 2 types of unit tests:
  - **interaction-based**: verifies that specific functions were called within the function of intestest.
  - **state-based**: check for the return value
...
- Will attempt to recreate this in vitest **later on** in my TanStack Start recreation.
- Given that the examples to go through depended on running tests against MongoDb and Mongoose, I will skip these.
- I encountered an issue with setting up Jest and handling environments, so I will generally forego this and later on try adding tests via vitest instead.
